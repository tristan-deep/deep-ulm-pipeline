...
# path = Path(__file__).parent.resolve() / 'KalmanNet' / 'checkpoints'
import numpy as np
import tensorflow as tf
from scipy.optimize import linear_sum_assignment
from sklearn.metrics import pairwise_distances

from tracking.kalmannet.KalmanNet_inference import KalmanNet

# from tracking.KalmanNet.KalmanNet_training import KalmanNetNN as Knet_full
from tracking.kalmannet.Linear_sysmdl import SystemModel


class timer(object):
    def __init__(self, k=3, print_time=False):
        self.t0 = time.time()
        self.k = k
...
class Tracks(object):
    ...
    def __init__(self, detection, trackId, m=4, n=2, hidden_dim_factor=1):
        super(Tracks, self).__init__()
        detection_in, detection = self.split_detection(detection)
        if type(detection) == np.ndarray:
            detection = detection.astype("float32")
        else:
            detection = detection.numpy().astype("float32")

        # Initialize KalmanNet parameters
        self.y_prev = tf.zeros(2)
        self.m1x_prior = tf.zeros((m, 1))
        self.m1x_posterior = tf.zeros((m, 1))
        self.KGain = tf.zeros((m, n))
        hidden_dim = (m * m + n * n) * hidden_dim_factor
        self.hn = tf.random.normal((hidden_dim,))
        self.prediction = detection[:2].reshape(1, 2)
        self.detection = detection
        # Initialize other parameters
        self.trackId = trackId
        self.skipped_frames = 0
        self.detectionsHist = [detection_in]
        state = np.zeros(4)
        state[[0, 2]] = detection
        self.statesHist = [state]
        self.predictionsHist = [self.prediction]

    def update(
        self, detection, output_Knet
    ):  # m1x_prior, m1x_posterior, KGain, prediction, hn):
        # Set outputs of KalmanNet
        m1x_prior, m1x_posterior, KGain, hn, prediction = output_Knet
        ...
        # Update history
        self.y_prev = self.detectionsHist[-1][1:3]
        self.statesHist.append(m1x_posterior.numpy().squeeze())
        if self.detectionsHist[-1][0] != detection[0]:  # not sure if still needed
            self.detectionsHist.append(detection)
        self.predictionsHist.append(self.prediction.numpy().squeeze())

    def getPrediction(self, k=3):
        # Return predicted next MB location
        if not len(self.detectionsHist):
            out = self.detection
        if len(self.detectionsHist) < k:
            out = self.detectionsHist[-1][1:3]
        else:
            out = self.prediction.numpy()
        return out

    def split_detection(self, detection_in):
        return detection_in, detection_in[1:3].reshape((1, 2))


# %% Tracker
class Tracker(object):
    """
    Class for the KalmanNet based tracking of MBs
    ...
    def __init__(
        ...
        trackId=0,
        initWindow=3,
        weights_checkpoint="KNet_06_27_22_L1_04_L2_16_HN_01",
    ):  # KNet_06_27_22_L1_04_L2_16_HN_01   KNet_06_26_22_L1_08_L2_16_HN_02  KNet_06_27_22_L1_01_L2_04_HN_01
        """
        ...
        # Initialize KalmanNet (parallel)
        self.sys_model = SystemModel(dt=dt)
        self.path_weights = (
            Path(__file__).parent.resolve() / "checkpoints" / weights_checkpoint
        )  # Save tracker in kalmannet
        self.Knet = KalmanNet()
        self.Knet.Build(self.path_weights, self.sys_model)

        # self.Knet_full = Knet_full()
        # self.Knet_full.Build(self.path_weights, self.sys_model)
    ...
    def update(self, detectionsInput):
        ...
        # Assignment of tracks in first frame
        if len(self.tracks) == 0:
            for i in range(detectionsInput.shape[0]):
                track = Tracks(detectionsInput[i], self.trackId)
                self.trackId += 1
                self.tracks.append(track)

        else:
            # Assignment with Hungarian Algorithn (linear sum assignment)
            N = len(self.tracks)
            M = len(detections)
            cost = []
            ...
            # Update tracks
            inputs = self.get_Knet_inputs(detections, assignment)
            if not (np.array(assignment) == -1).all():
                outputs = self.Knet(inputs)  # prior, posterior, KGain, hn, prediction
                k = 0
                for i in range(len(assignment)):
                    if assignment[i] != -1:
                        self.tracks[i].skipped_frames = 0
                        output_knet = [outputs[j][k] for j in range(len(outputs))]
                        self.tracks[i].update(
                            detectionsInput[assignment[i]], output_knet
                        )  # update
                        k += 1
            ...
            # Initialize new tracks for unassigned MBs
    ...
    def get_Knet_inputs(self, detections, assignment):
        ...
        hn_list = []
        y_prev_list = []
        lists = [prior_list, posterior_list, KGain_list, hn_list, y_prev_list]
        attributes = ["m1x_prior", "m1x_posterior", "KGain", "hn", "y_prev"]
        y_list = []

        for i, track in enumerate(self.tracks):
            if assignment[i] != -1:
                [
                    List.append(track.__getattribute__(attributes[j]))
                    for j, List in enumerate(lists)
                ]
                y_list.append(detections[assignment[i]])  # check this

        lists = [prior_list, posterior_list, KGain_list, hn_list, y_prev_list]
        [m1x_prior, m1x_posterior, KGain, hn, y_prev] = [
            tf.stack(List, axis=0) for List in lists
        ]
        y = tf.stack(y_list, axis=0)  # check dimensions

        return [y, y_prev, m1x_prior, m1x_posterior, KGain, hn]
