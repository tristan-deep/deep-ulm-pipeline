...
class Tracks(object):
    ...
    def __init__(self, detection, trackId, **kwargs):
        super().__init__()
...
class Tracker(object):
    ...
    def update(self, detectionsInput):
        ...
        if len(self.tracks) == 0:
            ...
            predictions = np.array(
                [track.getPrediction(k=self.initWindow) for track in self.tracks]
            ).squeeze()
    ...
    def retrieveTracks(
        ...
        mode_vel : str, optional
            ...
            calculated from the MB positions. The default is 'state'.
        ...
        for track in tracks:
            N = len(track.detectionsHist)  # Number of points on track
            if N > min_length - 1:
                ...
                trackNew[:, :2] = track.detectionsHist[:N, 1:3].copy().squeeze()  # z,x
                # else:
                #     raise(NotImplementedError("mode_MB_loc not specified, please set mode_MB_loc to one of the following: 'localization'"))

                if N > 1:
                    # Set MB velocities
                    # if mode_vel == 'calculate':     # Calculate velocity from MB positions
                    # if N > 2:
                    trackNew[1:, 2:4] = (
                    ...
                    # else:
                    #     raise(NotImplementedError("mode_vel not specified, please set mode_vel to one of the following: 'calculate'"))
